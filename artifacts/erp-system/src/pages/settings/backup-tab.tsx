...
import {
  ...
  CheckCircle2, AlertTriangle, X, Check, Database, Trash2, Clock, LogIn, LogOut,
} from "lucide-react";
...
export default function BackupTab() {
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      if (r.ok) {
        const d = await r.json() as { schedule: string; destination: string; last_scheduled: string | null; on_login: boolean; on_logout: boolean };
  ...
  const handleSaveSettings = async (overrides: { schedule?: string; destination?: string; on_login?: boolean; on_logout?: boolean }) => {
    setSchedSaving(true);
    try {
      const payload = {
      ...
      const r = await authFetch(api("/api/backups/settings"), { method: "PUT", body: JSON.stringify(payload) });
      if (r.ok) {
  ...
  const triggerBadge = (trigger: string) => {
    const cls = trigger === "login" ? "bg-blue-500/15 text-blue-400" : trigger === "logout" ? "bg-purple-500/15 text-purple-400" : trigger === "sale_post" || trigger === "purchase_post" ? "bg-emerald-500/15 text-emerald-400" : trigger === "scheduled" ? "bg-sky-500/15 text-sky-400" : "bg-white/8 text-white/40";
  ...
  const Toggle = ({ checked, onChange, disabled }: { checked: boolean; onChange: (v: boolean) => void; disabled?: boolean }) => (
    <button type="button" disabled={disabled} onClick={() => onChange(!checked)}
      ...
      <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all duration-200 ${checked ? "translate-x-[-0.125rem] left-5" : "left-0.5"}`} />
    </button>
  );
    ...
    <div className="space-y-6" dir="rtl">
      ...
      <div className="bg-[#111827] border border-white/8 rounded-2xl overflow-hidden">
        ...
        <div className="flex border-b border-white/8">
          {([["local", "💾 نسخة محلية", "تنزيل + استعادة"], ["server", "☁️ نسخة الخادم", "جدولة + حفظ + سجل"]] as const).map(([id, label, sub]) => (
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-2">
                {[{ v: "none", l: "بدون" }, { v: "daily", l: "يومياً" }, { v: "weekly", l: "أسبوعياً" }, { v: "monthly", l: "شهرياً" }].map(s => (
                  <button key={s.v} onClick={() => void handleSaveSettings({ schedule: s.v })} disabled={schedSaving}
                    ...
                    {schedSaving && schedule === s.v ? <Loader2 className="w-3 h-3 animate-spin inline ml-1" /> : null}
                    {s.l}
                  </button>
                ))}
              </div>
            ...
            <div className="rounded-xl border border-white/8 overflow-hidden">
              <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between bg-white/2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-white/25 text-[10px]">{backupList.length > 0 ? `${backupList.length} نسخة` : "لا توجد نسخ"}</span>
                </div>
                <button onClick={() => void loadList()} disabled={listLoading} className="p-1.5 rounded-lg text-white/25 hover:text-white hover:bg-white/8 transition-colors">
              ...
              {listLoading && backupList.length === 0 ? (
                ...
                <div className="divide-y divide-white/4 max-h-56 overflow-y-auto">
                  {backupList.map(b => (
                    <div key={b.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-white/2 transition-colors">
                      <div className="flex-1 min-w-0">
