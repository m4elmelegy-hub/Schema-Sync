...
import { useGetProducts, useGetSales, useGetPurchases, useGetSettingsSafes } from "@workspace/api-client-react";
...
import {
  ...
  Trash2, CheckCircle, XCircle, HandCoins, ArrowUpFromLine,
  ReceiptText, Wallet, ChevronRight, ArrowLeftRight,
} from "lucide-react";
...
/* ══════════════════════════════════════════════════════════════════════════════
   VOUCHERS HISTORY REPORT — سجل السندات
   All receipts + payments + transfers in one unified table
══════════════════════════════════════════════════════════════════════════════ */

interface RV { id: number; voucher_no: string; date: string; customer_id: number | null; customer_name: string; safe_id: number; safe_name: string; amount: number; notes: string | null; }
interface DV { id: number; voucher_no: string; date: string; customer_id: number | null; customer_name: string | null; safe_id: number; safe_name: string; amount: number; posting_status: string; source: string | null; notes: string | null; }
interface PV { id: number; voucher_no: string; date: string; customer_id: number | null; customer_name: string; safe_id: number; safe_name: string; amount: number; posting_status: string; notes: string | null; }
interface ST { id: number; type: string; safe_id: number | null; safe_name: string | null; amount: number; direction: string; description: string | null; date: string | null; }

type VKind = "receipt" | "deposit" | "payment" | "transfer";
interface UV { uid: string; kind: VKind; rawId: number; voucherNo: string; partyName: string; safeName: string; amount: number; status: string | null; date: string; notes: string | null; voucherType: "قبض" | "صرف"; subType: string; }
...
function unifyVouchers(r: RV[], d: DV[], p: PV[], t: ST[]): UV[] {
  ...
  t.filter(x => x.direction === "out" && x.type === "transfer_out").forEach(x => rows.push({ uid:`transfer-${x.id}`, kind:"transfer", rawId:x.id, voucherNo:`TRF-${x.id}`, partyName:x.description || "تحويل خزنة", safeName:x.safe_name || "—", amount:x.amount, status:null, date:x.date || "", notes:null, voucherType:"صرف", subType:"تحويل خزنة" }));
...
function VSubBadge({ sub }: { sub: string }) {
  const map: Record<string, string> = { "عميل": "bg-blue-500/15 text-blue-300 border-blue-500/20", "توريد": "bg-teal-500/15 text-teal-300 border-teal-500/20", "صرف": "bg-orange-500/15 text-orange-300 border-orange-500/20", "تحويل خزنة": "bg-violet-500/15 text-violet-300 border-violet-500/20" };
...
type VFilter = "الكل" | "قبض" | "صرف";

function VouchersHistoryReport() {
  const qc = useQueryClient();
  const { toast } = useToast();
  const { data: safes = [] } = useGetSettingsSafes();

  const { data: receipts  = [], isLoading: l1 } = useQuery<RV[]>({ queryKey:["/api/receipt-vouchers"],  queryFn: () => authFetch(api("/api/receipt-vouchers")).then(r  => { if (!r.ok) throw new Error("خطأ"); return r.json(); }) });
  const { data: deposits  = [], isLoading: l2 } = useQuery<DV[]>({ queryKey:["/api/deposit-vouchers"],  queryFn: () => authFetch(api("/api/deposit-vouchers")).then(r  => { if (!r.ok) throw new Error("خطأ"); return r.json(); }) });
  const { data: payments  = [], isLoading: l3 } = useQuery<PV[]>({ queryKey:["/api/payment-vouchers"],  queryFn: () => authFetch(api("/api/payment-vouchers")).then(r  => { if (!r.ok) throw new Error("خطأ"); return r.json(); }) });
  const { data: transfers = [], isLoading: l4 } = useQuery<ST[]>({ queryKey:["/api/safe-transfers"],    queryFn: () => authFetch(api("/api/safe-transfers")).then(r    => { if (!r.ok) throw new Error("خطأ"); return r.json(); }) });
  const isLoading = l1 || l2 || l3 || l4;

  const [vFilter, setVFilter]       = useState<VFilter>("الكل");
  const [confirmDel, setConfirmDel] = useState<{ uid: string; kind: VKind; rawId: number } | null>(null);

  const allRows  = useMemo(() => unifyVouchers(receipts, deposits, payments, transfers), [receipts, deposits, payments, transfers]);
  const filtered = vFilter === "الكل" ? allRows : allRows.filter(r => r.voucherType === vFilter);

  const delReceipt = useMutation({ mutationFn: (id: number) => authFetch(api(`/api/receipt-vouchers/${id}`), { method:"DELETE" }).then(r => { if (!r.ok) throw new Error("فشل الحذف"); }), onSuccess: () => { qc.invalidateQueries({queryKey:["/api/receipt-vouchers"]}); qc.invalidateQueries({queryKey:["/api/settings/safes"]}); toast({title:"تم الحذف"}); setConfirmDel(null); }, onError: (e: Error) => toast({title:e.message, variant:"destructive"}) });
  const delDeposit = useMutation({ mutationFn: (id: number) => authFetch(api(`/api/deposit-vouchers/${id}`), { method:"DELETE" }).then(async r => { if (!r.ok) { const e = await r.json(); throw new Error(e.error||"فشل"); } }), onSuccess: () => { qc.invalidateQueries({queryKey:["/api/deposit-vouchers"]}); qc.invalidateQueries({queryKey:["/api/settings/safes"]}); toast({title:"تم الحذف"}); setConfirmDel(null); }, onError: (e: Error) => toast({title:e.message, variant:"destructive"}) });
  const delPayment = useMutation({ mutationFn: (id: number) => authFetch(api(`/api/payment-vouchers/${id}`), { method:"DELETE" }).then(async r => { if (!r.ok) { const e = await r.json(); throw new Error(e.error||"فشل"); } }), onSuccess: () => { qc.invalidateQueries({queryKey:["/api/payment-vouchers"]}); qc.invalidateQueries({queryKey:["/api/settings/safes"]}); toast({title:"تم الحذف"}); setConfirmDel(null); }, onError: (e: Error) => toast({title:e.message, variant:"destructive"}) });
  ...
  function handleDelete() {
    ...
    if (confirmDel.kind === "receipt") delReceipt.mutate(confirmDel.rawId);
    if (confirmDel.kind === "deposit") delDeposit.mutate(confirmDel.rawId);
    if (confirmDel.kind === "payment") delPayment.mutate(confirmDel.rawId);
  }
  ...
  return (
    <div className="space-y-6" dir="rtl">
      {confirmDel && (
        <ConfirmModal title="حذف السند" description="سيتم حذف السند وعكس أثره على الخزينة."
          isPending={delReceipt.isPending || delDeposit.isPending || delPayment.isPending}
      ...
      <div className="flex items-center gap-1 bg-white/5 border border-white/10 rounded-2xl p-1 w-fit">
        {(["الكل", "قبض", "صرف"] as VFilter[]).map(t => {
          const count = t === "الكل" ? allRows.length : allRows.filter(r => r.voucherType === t).length;
          return (
            <button key={t} onClick={() => setVFilter(t)}
              className={`flex items-center gap-1.5 px-5 py-2 rounded-xl text-sm font-bold transition-all ${vFilter === t ? "bg-amber-500 text-black shadow-lg" : "text-white/50 hover:text-white"}`}>
              ...
              {t}
              <span className={`text-xs px-1.5 py-0.5 rounded-full ${vFilter === t ? "bg-black/20" : "bg-white/10"}`}>{count}</span>
            </button>
          );
        })}
      </div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={9} className="p-16 text-center">
                    ...
                    <p className="text-white/40 text-sm">لا توجد سندات</p>
                    <p className="text-white/25 text-xs mt-1">أنشئ السندات من صفحة الخزينة</p>
                  </td>
                </tr>
              ) : filtered.map(row => (
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1">
                      {row.kind === "deposit" && row.status === "draft" && (
                        <button onClick={() => postDeposit.mutate(row.rawId)} disabled={postDeposit.isPending} title="ترحيل" className="btn-icon text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10"><CheckCircle className="w-4 h-4" /></button>
                      )}
                      {row.kind === "deposit" && row.status === "posted" && (
                        <button onClick={() => cancelDeposit.mutate(row.rawId)} disabled={cancelDeposit.isPending} title="إلغاء" className="btn-icon text-amber-400 hover:text-amber-300 hover:bg-amber-500/10"><XCircle className="w-4 h-4" /></button>
                      )}
                      {row.kind === "payment" && row.status === "draft" && (
                        <button onClick={() => postPayment.mutate(row.rawId)} disabled={postPayment.isPending} title="ترحيل" className="btn-icon text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10"><CheckCircle className="w-4 h-4" /></button>
                      )}
                      {row.kind === "payment" && row.status === "posted" && (
                        <button onClick={() => cancelPayment.mutate(row.rawId)} disabled={cancelPayment.isPending} title="إلغاء" className="btn-icon text-amber-400 hover:text-amber-300 hover:bg-amber-500/10"><XCircle className="w-4 h-4" /></button>
                      )}
                      {row.kind !== "transfer" && row.status !== "posted" && (
                        <button onClick={() => setConfirmDel({ uid:row.uid, kind:row.kind, rawId:row.rawId })} className="btn-icon btn-icon-danger" title="حذف"><Trash2 className="w-4 h-4" /></button>
                      )}
                    </div>
                  </td>
                </tr>
...
type Tab = "pl" | "daily" | "products" | "analysis" | "customer" | "cashflow" | "top" | "inventory" | "purchases" | "sales" | "health" | "vouchers";
