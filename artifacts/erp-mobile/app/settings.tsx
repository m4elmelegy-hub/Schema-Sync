...
export default function SettingsScreen() {
  ...
  const { mutate: deleteWarehouse } = useMutation({
    mutationFn: (id: number) =>
      apiFetch(`/api/settings/warehouses/${id}`, { method: "DELETE" }),
  ...
  const { mutate: deleteSafe } = useMutation({
    mutationFn: (id: number) =>
      apiFetch(`/api/settings/safes/${id}`, { method: "DELETE" }),
  ...
  return (
    <View style={[styles.container, { backgroundColor: c.background }]}>
            ...
            <FlatList
              ...
              renderItem={({ item: wh }) => (
                <View style={[styles.listCard, { backgroundColor: c.card, borderColor: c.cardBorder }]}>
                  <TouchableOpacity
                    onPress={() => isAdmin && confirmDelete(wh.name, () => deleteWarehouse(wh.id))}
                    style={styles.deleteBtn}
                  >
                    {isAdmin && <Feather name="trash-2" size={16} color="#EF4444" />}
                  </TouchableOpacity>
              ...
              ListEmptyComponent={
                <View style={styles.empty}>
                  ...
                  <Text style={[styles.emptyText, { color: c.mutedForeground }]}>لا توجد مخازن</Text>
                </View>
              }
            />
          )}
          ...
          {tab === "safes" && (
            <FlatList
              ...
              renderItem={({ item: s }) => (
                <View style={[styles.listCard, { backgroundColor: c.card, borderColor: c.cardBorder }]}>
                  <TouchableOpacity
                    onPress={() => isAdmin && confirmDelete(s.name, () => deleteSafe(s.id))}
                    style={styles.deleteBtn}
                  >
                    {isAdmin && <Feather name="trash-2" size={16} color="#EF4444" />}
                  </TouchableOpacity>
              ...
              ListEmptyComponent={
                <View style={styles.empty}>
                  <Feather name="dollar-sign" size={32} color={c.mutedForeground} />
                  <Text style={[styles.emptyText, { color: c.mutedForeground }]}>لا توجد خزائن</Text>
                </View>
              }
            />
          )}
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <View style={styles.empty}>
                  ...
                  <Text style={[styles.emptyText, { color: c.mutedForeground }]}>لا يوجد مستخدمون</Text>
                </View>
              }
            />
          )}
      ...
      <AddModal
        ...
        title="إضافة مستخدم"
        fields={[
          { key: "name", label: "الاسم الكامل", placeholder: "مثال: أحمد محمد" },
          { key: "username", label: "اسم المستخدم", placeholder: "مثال: ahmed" },
          { key: "pin", label: "الرقم السري (6 أرقام)", placeholder: "123456", keyboardType: "numeric" },
          { key: "role", label: "الدور (cashier/salesperson/admin/manager)", placeholder: "cashier" },
        ]}
        onClose={() => setModal(null)}
        onSubmit={addUser}
        loading={addingUs}
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  tabBar: {
    flexDirection: "row-reverse", borderBottomWidth: 1,
  ...
  addBtn: {
    ...
    borderStyle: "dashed", padding: 14, marginBottom: 4,
