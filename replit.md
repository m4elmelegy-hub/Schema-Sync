...
- **Period Lock (قفل الفترة):** A `closing_date` setting (stored in `system_settings` table) blocks any write/delete/cancel on documents whose date ≤ closing date. Checked via `assertPeriodOpen(docDate, req)` in all financial write handlers (sales, purchases, returns, receipt/payment/deposit vouchers). Admin users may pass `{ admin_override: true }` in the body to bypass. Managed via `GET /settings/period` (public) and `PUT /settings/period` (admin only). Cache TTL of 5 s prevents per-request DB reads.
