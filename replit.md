...
- **SaaS Multi-Tenant Platform:** Full SaaS transformation with company registration, trial subscriptions, email/password auth, and super admin dashboard.
  ...
  - **Company-ID filtering:** `GET /expenses`, `GET /income`, `GET /settings/safes`, `GET /settings/warehouses` now filter by `req.user.company_id`. All corresponding `POST` endpoints stamp `company_id` on new records.
  ...
  - **Super Admin Dashboard:** `artifacts/erp-system/src/pages/super-admin.tsx` — full-screen panel showing stats cards (total, active, trial, expired, suspended companies + user count), company table with status pills, days remaining, expand/collapse rows with actions (activate, suspend, extend, upgrade plan). Loaded automatically for `super_admin` role; bypasses normal AppLayout.
  - **Subscription system retained:** Companies table, company_id filtering already in sales/products/purchases routes, SubscriptionBanner, JWT auth — all pre-existing and preserved.
  - **Default super_admin credentials:** email `super@halal-tech.com` / password `superadmin123` (seeded via SQL on first setup).
