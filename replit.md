...
- **SaaS Multi-Tenant Platform:** Full SaaS transformation with company registration, trial subscriptions, email/password auth, and super admin dashboard.
  ...
  - **Company-ID filtering:** All data routes filter by `req.user.company_id`. All write endpoints stamp `company_id` on new records. Alerts and audit_logs now also filtered.
