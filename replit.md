...
- **Inventory System:** Implements a double-entry inventory tracking system via `stock_movements` table, supporting movement types (`opening_balance`, `purchase`, `sale`, `sale_return`, `purchase_return`, `adjustment`, `transfer_in`, `transfer_out`). Calculates `current_qty` and checks for discrepancies. Warehouse management is now consolidated in `inventory.tsx` (card-based UI with add/delete modals, backend DELETE validates no stock movements/count sessions/transfers). `products.tsx` renders only `ProductsTab` directly. Settings no longer has a WarehousesTab.
